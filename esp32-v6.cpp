...
#define FRONT_LEFT_PWM_CH   0    //PWM pin สำหรับความเร็วมอเตอร์หน้าซ้าย
#define FRONT_RIGHT_PWM_CH  1    //PWM pin สำหรับความเร็วมอเตอร์หน้าขวา
#define FRONT_LEFT_DIR_PIN  32   //มอเตอร์หน้าซ้าย
#define FRONT_RIGHT_DIR_PIN 33   //มอเตอร์หน้าขวา
#define FRONT_PWM_PIN       18   // ใช้ pin เดียวสำหรับ PWM (ESP32 สามารถ attach หลาย channel ได้)

#define REAR_LEFT_PWM_CH    2    //PWM pin สำหรับความเร็วมอเตอร์หลังซ้าย
#define REAR_RIGHT_PWM_CH   3    //PWM pin สำหรับความเร็วมอเตอร์หลังขวา
#define REAR_LEFT_DIR_PIN   25   //มอเตอร์หลังซ้าย
#define REAR_RIGHT_DIR_PIN  26   //มอเตอร์หลังขวา
#define REAR_PWM_PIN        19   // ใช้ pin เดียวสำหรับ PWM (ESP32 สามารถ attach หลาย channel ได้)
...
void stopMotors();
void setMotorAxle(int pwmVal,int dir,int leftChannel,int leftDir,int rightChannel,int rightDir);
void setMotorSide(int pwmVal,int dir,int pwmChannel,int dirPin);
...
void setup() {
  ...
  ledcSetup(FRONT_LEFT_PWM_CH, 20000, 8); ledcAttachPin(FRONT_PWM_PIN, FRONT_LEFT_PWM_CH); ledcWrite(FRONT_LEFT_PWM_CH,0);
  ledcSetup(FRONT_RIGHT_PWM_CH,20000, 8); ledcAttachPin(FRONT_PWM_PIN, FRONT_RIGHT_PWM_CH); ledcWrite(FRONT_RIGHT_PWM_CH,0);
  ledcSetup(REAR_LEFT_PWM_CH,   20000, 8); ledcAttachPin(REAR_PWM_PIN,  REAR_LEFT_PWM_CH);  ledcWrite(REAR_LEFT_PWM_CH,0);
  ledcSetup(REAR_RIGHT_PWM_CH,  20000, 8); ledcAttachPin(REAR_PWM_PIN,  REAR_RIGHT_PWM_CH); ledcWrite(REAR_RIGHT_PWM_CH,0);

  pinMode(FRONT_LEFT_DIR_PIN, OUTPUT); pinMode(FRONT_RIGHT_DIR_PIN, OUTPUT);
  pinMode(REAR_LEFT_DIR_PIN, OUTPUT);  pinMode(REAR_RIGHT_DIR_PIN, OUTPUT);
  ...
  for(;;){
    ...
    if(!emergencyInterrupt){
      ...
      if(strcmp(motionState,"Left")==0){
        ...
        setMotorSide(requestedSpeed,1,FRONT_RIGHT_PWM_CH,FRONT_RIGHT_DIR_PIN);
        setMotorSide(0,0,FRONT_LEFT_PWM_CH,FRONT_LEFT_DIR_PIN);
        setMotorAxle(requestedSpeed,1,REAR_LEFT_PWM_CH,REAR_LEFT_DIR_PIN,REAR_RIGHT_PWM_CH,REAR_RIGHT_DIR_PIN);
      } else if(strcmp(motionState,"Right")==0){
        // เลี้ยวขวา: หยุดล้อหน้า-ขวา, ล้อหน้า-ซ้าย + หลัง ขับ
        setMotorSide(requestedSpeed,1,FRONT_LEFT_PWM_CH,FRONT_LEFT_DIR_PIN);
        setMotorSide(0,0,FRONT_RIGHT_PWM_CH,FRONT_RIGHT_DIR_PIN);
        setMotorAxle(requestedSpeed,1,REAR_LEFT_PWM_CH,REAR_LEFT_DIR_PIN,REAR_RIGHT_PWM_CH,REAR_RIGHT_DIR_PIN);
      } else {
...
void stopMotors(){ 
  setMotorAxle(0,0, FRONT_LEFT_PWM_CH, FRONT_LEFT_DIR_PIN, FRONT_RIGHT_PWM_CH, FRONT_RIGHT_DIR_PIN);
  setMotorAxle(0,0, REAR_LEFT_PWM_CH,  REAR_LEFT_DIR_PIN,  REAR_RIGHT_PWM_CH,  REAR_RIGHT_DIR_PIN);
}

// ตั้งค่า axle (หน้า+หลัง)
void setAxles(int speedFront,int dirFront,int speedRear,int dirRear){
  setMotorAxle(speedFront, dirFront, FRONT_LEFT_PWM_CH, FRONT_LEFT_DIR_PIN, FRONT_RIGHT_PWM_CH, FRONT_RIGHT_DIR_PIN);
  setMotorAxle(speedRear,  dirRear,  REAR_LEFT_PWM_CH,  REAR_LEFT_DIR_PIN,  REAR_RIGHT_PWM_CH,  REAR_RIGHT_DIR_PIN);
}
...
void setMotorAxle(int pwmVal,int dir,int leftChannel,int leftDir,int rightChannel,int rightDir){
  ...
  if(dir==0){
      digitalWrite(leftDir,LOW);  digitalWrite(rightDir,LOW);
      ...
      digitalWrite(leftDir,LOW); digitalWrite(rightDir,LOW); // backward
      ledcWrite(leftChannel,pwmVal); ledcWrite(rightChannel,pwmVal);
  }
}
...
void setMotorSide(int pwmVal,int dir,int pwmChannel,int dirPin){
  pwmVal=constrain(pwmVal,0,255);
  if(dir==0){ digitalWrite(dirPin,LOW); ledcWrite(pwmChannel,0); }           // stop → DIR1=LOW, DIR2=LOW
  else if(dir==1){ digitalWrite(dirPin,HIGH); ledcWrite(pwmChannel,pwmVal); }// forward → DIR1=HIGH, DIR2=LOW
  else if(dir==2){ digitalWrite(dirPin,LOW); ledcWrite(pwmChannel,pwmVal); } // backward → DIR1=LOW, DIR2=HIGH
